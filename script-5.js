...
(function() {
    ...
    GM_addStyle(`
        ...
        /* MODIFICATION: Styles for collapsing replies */
    ...
    // MODIFICATION: Logic for reply toggling and default collapsed state added.
    function createCommentElement(comment, isReply = false) {
        ...
        // Replies container is collapsed by default if replies exist
        ...
        if (hasReplies) {
            ...
            repliesToggleHTML = `
                <button class="replies-toggle-button" data-action="toggle-replies">
                    &#9473;&#9473; View ${comment.replies.length} ${replyText}
                </button>`;
        }
    ...
    // MODIFICATION: Preserve expanded state of replies during re-render.
    function renderComment(comment) {
        ...
        if (el) {
            ...
            // Preserve the expanded/collapsed state before replacing the element
            const oldRepliesContainer = el.querySelector('.social-replies-container');
            const wasExpanded = oldRepliesContainer && !oldRepliesContainer.classList.contains('collapsed');
            ...
            if (!isReply && comment.replies && comment.replies.length > 0) {
                ...
                repliesContainer.innerHTML = ''; // Clear old replies before re-adding
                comment.replies.forEach(reply => {
                    repliesContainer.appendChild(createCommentElement(reply, true));
                });
            }
            ...
            if (wasExpanded) {
                const newRepliesContainer = newEl.querySelector('.social-replies-container');
                const toggleButton = newEl.querySelector('[data-action="toggle-replies"]');
                if (newRepliesContainer) newRepliesContainer.classList.remove('collapsed');
                if (toggleButton) toggleButton.innerHTML = '&#9473;&#9473; Hide replies';
            }
        }
    }
    ...
    // MODIFICATION: Added 'toggle-replies' action handler.
...
setInterval(() => {
    ...
    // 1) Live‐timestamp thread updates
    const currentSecond = Math.floor(state.currentTime);
    if (
      state.isOpen &&
      state.trackId &&
      state.sortMode === 'timestamp' &&
      currentSecond !== state.lastFetchedSecond
    ) {
        state.lastFetchedSecond = currentSecond;
        fetchComments({ threadStartsAt: currentSecond });
    }

    // 2) DOM-based track ID detection
    const footerLink = document.querySelector(
      '#footerPlayer > div._leftColumn_aaf28de > div._currentMediaItemDetails_dc0f159 > div._trackHeaderContainer_b72c901 > div._trackTitleContainer_9b35d33 > a'
    );
    const newTrackId = footerLink
      ?.getAttribute('href')
      ?.match(/\/track\/(\d+)/)
      ?.[1] || null;
