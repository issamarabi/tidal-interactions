/**
 * POST /comments-remove-comment-reaction
 *
 * Request JSON:
 * {
 ...
 */ import { serve } from "https://deno.land/std@0.184.0/http/server.ts";
 ...
 serve(async (req)=>{
   ...
   const { data: existing, error: selectError } = await supabase.from("comment_reactions").select("tidal_user_id,comment_id,emoji").eq("tidal_user_id", tidal_user_id).eq("comment_id", comment_id).eq("emoji", emoji).single();
   if (selectError) {
   ...
   const { error } = await supabase.from("comment_reactions").delete().eq("tidal_user_id", tidal_user_id).eq("comment_id", comment_id).eq("emoji", emoji);
   if (error) {
