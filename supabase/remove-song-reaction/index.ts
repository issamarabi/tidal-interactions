/**
 * POST /remove-song-reaction
 *
 * Request JSON:
 * {
 *   "id": string    // reaction ID to delete
 * }
 *
 * Success (200):
 * {
 *   "id": string,          // deleted reaction ID
 *   "deleted": boolean     // always true if deletion succeeded
 * }
 *
 * Errors:
 *   400 Bad Request    { "error": "Missing fields: id" }
 *   404 Not Found      { "error": "Song reaction not found" }
 ...
 */ import { serve } from "https://deno.land/std@0.184.0/http/server.ts";
 ...
 serve(async (req)=>{
   ...
   const { data: existing, error: selectError } = await supabase.from("reactions").select("id").eq("id", id).single();
   if (selectError) {
     if (selectError.code === "PGRST116") {
       return new Response(JSON.stringify({
         error: "Song reaction not found"
       }), {
         status: 404
       });
     }
