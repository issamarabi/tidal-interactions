...
(function() {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                ...
                if (state.sortMode === 'timestamp') {
                    ...
                    ).sort((a, b) => a.playback_time - b.playback_time);
                } else {
    ...
    function registerUserOnce() {
        ...
        if (state.registered) return Promise.resolve();
          ...
          GM_xmlhttpRequest({
            ...
            onload: res => {
              if (res.status === 200 || res.status === 201) {
                ...
                console.log('[SC] registered → true');
              } else {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                if (response.status === 201) {
                    ...
                    // If it's a reply, make sure the parent comment's replies are expanded
                    if (isReply && parentId) {
                        state.expandedComments.add(parentId);
                    }
            ...
            <div class="social-comments-header _header_f4bacf5">
                ...
                <div class="social-header-controls">
                    <select id="social-sort-mode" class="social-sort-select">
                        ...
                        <option value="most_reactions">Most Liked</option>
    ...
    function createCommentElement(comment, isReply = false) {
        ...
        // Check if this comment should be expanded based on state
        const shouldBeExpanded = hasReplies && state.expandedComments.has(comment.id);
        const repliesContainerClass = hasReplies ?
            (shouldBeExpanded ? 'social-replies-container' : 'social-replies-container collapsed') :
            'social-replies-container';

        let repliesToggleHTML = '';
        if (hasReplies) {
            const replyText = comment.replies.length === 1 ? 'reply' : 'replies';
            const toggleText = shouldBeExpanded ?
                `&#9473;&#9473; Hide replies` :
                `&#9473;&#9473; View ${comment.replies.length} ${replyText}`;
            repliesToggleHTML = `<button class="replies-toggle-button" data-action="toggle-replies">${toggleText}</button>`;
        }
    ...
    setInterval(() => {
        ...
        if (newTrackId !== lastTrackId) {
            ...
            state.allTimestampComments = [];
            // Clear expanded comments state for new track
